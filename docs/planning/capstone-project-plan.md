...
| **Cloud MVP** | End of S9 | The same workflow deployed repeatably to an initial EC2 target provisioned with Terraform and PostgreSQL state on Amazon RDS, after a tested SQLite migration and intentional CI/CD promotion, with access controls, workload identities, secrets handling, observability, cost controls, failure recovery and verified teardown/recreation. |
...
Do not introduce deep learning, multi-agent orchestration, Kubernetes, Kafka, a separate vector database, caching or an elaborate dashboard without evidence. Implementation uses the TRACE repository and a modular Python application. Packaging/runtime details, persistence and model/provider choices require justified ADRs. Terraform and EC2 are selected for the Cloud MVP direction. [ADR 0005](../adr/0005-sqlite-local-postgresql-rds-cloud.md) selects SQLite for Local MVP state and a later PostgreSQL-on-RDS migration for Cloud MVP; exact AWS topology, database configuration and application-artifact release remain S7/S8 design decisions alongside [ADR 0002](../adr/0002-terraform-and-ec2-cloud-deployment.md). Existing lexical/semantic/hybrid retrieval is reused as a measured baseline. The owner selected PostgreSQL with pgvector on the planned RDS database for Cloud MVP; [ADR 0006](../adr/0006-pgvector-cloud-retrieval-storage.md) records that direction. Version compatibility, configuration and #16 retrieval evidence remain gates. New reranking or a separate vector service still needs evidence.
...
| 3 | Reliable evidence and early scoring | Bounded log/metric/deployment queries and SQLite persistence for investigation/evidence state; regression/degradation/insufficient-evidence fixtures; noncausal deployment distractor; UTC timeline; development scoring runner. | Query evidence by ID/window/service; reproducible manifests; deterministic timeline and scorer fixtures pass; reserve final-test data. | S2 |
| 4 | Small retrieval corpus and controlled tools | Curated versioned knowledge, labeled retrieval queries and Recall@K/MRR; preserve exact search as the Local MVP baseline and record cloud pgvector compatibility requirements; typed bounded log/metric/deployment/knowledge tools; timeout/audit/authorization contracts. | Known queries retrieve supporting documents; invalid tool calls rejected; no operational write capability. | S3 |
...
| 7 | Terraform and EC2 deployment design | Design a minimal Terraform-managed EC2 target and PostgreSQL-on-RDS state from measured MVP workload; decide region, AMI/instance and RDS sizing, network, Terraform state backend, identity/secrets, access, budget, backup/retention, SQLite migration, pgvector extension compatibility and teardown. | Reviewable EC2/RDS plan/configuration and SQLite migration plan with cost assumptions, least-privilege access, backup/restore and teardown; no unnecessary environment duplication or resources applied. | Local MVP gate |
| 8 | EC2 application deployment and CI/CD | Automate immutable application release to the Terraform-managed EC2 target and migrate durable state from SQLite to PostgreSQL on RDS and enable versioned pgvector retrieval storage after its design, access, cost and compatibility gates are met; select a reproducible image/bootstrap or other deployment mechanism, intentional promotion, workload identity, access controls, smoke tests, CloudWatch Logs configuration for TRACE application records (log group, retention and EC2 agent collection), minimal end-to-end logs/metrics and cost tracking. | Deploy and run local-MVP scenarios on EC2; confirm access isolation, evidence traceability, rollback/recovery and CI/CD smoke checks. | S7 |
| 9 | Cloud MVP acceptance | Failure recovery, prompt injection/poisoning checks, resource budgets, operational guide and teardown/recreation. | Demonstrate investigation, SQLite-to-RDS migration preserving evidence identities, database backup/restore, any approved vector-index rebuild with citation integrity, failed-request recovery, enforced read-only tools and resource removal/recreation; verify correlated, redacted application records in CloudWatch with configured retention, rotation and collector recovery; cloud release criteria evidenced. | S8 |
