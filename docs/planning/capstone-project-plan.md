...
| 8 | EC2 application deployment and CI/CD | Automate immutable application release to the Terraform-managed EC2 target; select a reproducible image/bootstrap or other deployment mechanism, intentional promotion, workload identity, access controls, smoke tests, CloudWatch Logs configuration for TRACE application records (log group, retention and EC2 agent collection), minimal end-to-end logs/metrics and cost tracking. | Deploy and run local-MVP scenarios on EC2; confirm access isolation, evidence traceability, rollback/recovery and CI/CD smoke checks. | S7 |
| 9 | Cloud MVP acceptance | Failure recovery, prompt injection/poisoning checks, resource budgets, operational guide and teardown/recreation. | Demonstrate investigation, failed-request recovery, enforced read-only tools and resource removal/recreation; verify correlated, redacted application records in CloudWatch with configured retention, rotation and collector recovery; cloud release criteria evidenced. | S8 |
