...
- **Vector storage direction:** the owner selected PostgreSQL with pgvector on the planned RDS database for Cloud MVP; [ADR 0006](adr/0006-pgvector-cloud-retrieval-storage.md) records the choice. Embedding/index design and #16 retrieval acceptance remain open. Keep the small exact-search Local MVP baseline and authorization/citation contracts.
