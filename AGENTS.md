...
TRACE is a read-only incident investigation assistant that helps an engineer gather and assess evidence. The product goal is evidence-supported diagnosis with provenance, competing hypotheses, missing information, uncertainty and abstention. Time correlation alone does not prove root cause.
...
- Keep Local MVP, Cloud MVP, Full Capstone and the independent Operational Pilot gates separate. Synthetic evaluation or cloud deployment does not prove operational usefulness.
...
- Keep operational evidence and interfaces separate from evaluator-only labels. Preserve provenance, access scope and bounded read-only behavior.
