...
- Work in the TRACE repository as one modular Python application. Current scaffold decisions are Python 3.12/Linux-first, `pyproject.toml`, `uv.lock`, and an empty runtime dependency set until needed; see ADR 0001. Terraform and EC2 are the approved Cloud MVP deployment direction; see ADR 0002 for deferred design choices. SQLite is selected for Local MVP investigation/evidence state, followed by PostgreSQL on Amazon RDS for Cloud MVP; see ADR 0005. Neither database is implemented or provisioned yet. PostgreSQL with pgvector on RDS is the selected Cloud MVP vector database; see ADR 0006. Retrieval configuration and acceptance still depend on measured evidence.
