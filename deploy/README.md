...
S7 chooses exact EC2/RDS topology, database access/backup, proposed pgvector extension compatibility and Terraform state; S8 migrates SQLite state to RDS, enables pgvector only if ADR 0006 is approved, chooses application artifact release/rollback and configures CloudWatch Logs collection on EC2; S9 verifies migration, restore, hosted delivery, retention, rotation and collector recovery as part of Cloud MVP acceptance. Until then, use the [local development workflow](../docs/development.md). Neither a plan file nor these documents prove hosted logging delivery or Cloud MVP acceptance.
